...
// 放大镜开始
function Magnifier() {
  var z_index = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 0;
  // 获取需要的元素
  // 小图元素
  this.wrap_small_ele = document.querySelector(".wrap-small-magnifier");
  this.wrap_small_img = document.querySelectorAll(".tong");
  this.wrap_small_img = document.querySelectorAll(".tong"); // 焦点

  this.focus_ele = document.querySelector(".focus"); // 大图

  this.wrap_big_ele = document.querySelector(".wrap-big-magnifier"); //大图big下的所有图

  this.big_img = this.wrap_big_ele.querySelectorAll("img"); //第一个大图

  this.big_img_one = this.wrap_big_ele.querySelector(".one"); //第二个大图

  this.big_img_tow = this.wrap_big_ele.querySelector(".tow"); // 选项图片

  this.btn_eles = document.querySelectorAll(".wrap-choice-magnifier i"); // 下标
...
Magnifier.prototype.bindEvent = function () {
  // 存实例对象；
  var magnifier_instance = this; // 鼠标移入事件
...
Magnifier.prototype.move = function (x, y) {
  ...
  var _top = y - 150; // 边界检测：判定数据的极值；


  _left = _left < 0 ? 0 : _left;
  _top = _top < 0 ? 0 : _top;
  _left = _left > 180 ? 180 : _left;
  _top = _top > 180 ? 180 : _top; // 给focus定位

  this.focus_ele.style.left = _left + 'px';
  this.focus_ele.style.top = _top + 'px'; // 大图运动：
  // 先计算小图left值和top的运动比例；
  // left/运动的最大值；

  var _left_prop = _left / 100;

  var _top_prop = _top / 100; // 根据比例让大图运动
...
$(function () {
  ...
  $('.btn_lft').find('button').click(function () {
    // 获取索引
    var index_m1 = $(this).index(); // 给当前触发元素添加color_m 并且删除兄弟级的color_m 

    $(this).addClass('color_m ').siblings().removeClass('color_m '); // 设置当前触发元素的显示及兄弟级的隐藏
